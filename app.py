...
# ---------------------- HEADER ----------------------
st.markdown(
    "<h1 style='text-align:center;'>🔮 Arganda Predictiva</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<h4 style='text-align:center;color:gray;'>Gemelo digital y predicción urbana basada en datos abiertos</h4>",
    unsafe_allow_html=True
)
st.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "Demo técnica • Datos reales cuando se configuren las API keys, "
    "datos simulados mientras tanto."
)

# ---------------------- DATA FETCH ----------------------
# AEMET API key (si está configurada en secrets)
try:
...
c4.metric(
    "Personas en paro",
    f"{unemp['last_value']:,}".replace(",", ".")
)

st.caption(
    "Los valores se calculan combinando datos (o simulaciones) de AEMET, DGT e INE."
)
...
st.caption(
    "Modelo simplificado que tiene en cuenta lluvia, temperatura, patrón semanal "
    "e intensidad de tráfico."
)
...
# FIX: garantizar que 'paro' tenga SIEMPRE 12 valores
if unemp.get("series") and len(unemp["series"]) >= 12:
    paro = np.array(unemp["series"][-12:])
else:
    paro = np.linspace(3800, 3300, 12) + np.random.normal(0, 40, 12)

# FIX: garantizar que traffic_incidents tenga SIEMPRE 12 valores
...
st.caption(
    "Este gráfico sirve para ilustrar cómo explorar relaciones entre paro y "
    "tráfico con datos abiertos reales."
)
...
if show_map:
    ...
    # st.map sólo puede usar lat/lon
    st.map(map_df[["lat", "lon"]], zoom=13)

    st.caption(
        "En una versión avanzada, cada punto representaría un cruce o tramo real "
